			...
			if (arg0.getActionCommand().equals("cancel")) {
				...
				if (!userField.getText().equals("")
					...
					try {
						name = JOptionPane
								.showInputDialog(
										"Your name & lastname (format: name lastname:")
								.trim().split(" ");
					...
					if (name == null || name.length != 2) {
						...
						JOptionPane
								.showMessageDialog(null,
										"please input both name and last name in correct format");
				...
				User user = Authentication.Authenticate(userField.getText(),
				...
				if (user == null) {
					...
					info.setText("User: null , Name: null , LastName: null ");
					System.out
							.println("No result found or database does not exist.");
				} else {
					info.setText("User: " + user.getUsername() + ", Name: "
							+ user.getName() + " , LastName: "
							+ user.getLastname());
